...
function CommandInput({
  ...
  return (
    <div data-slot="command-input-wrapper" className="p-2 pb-0">
      <InputGroup className="h-12! rounded-xl! border-input/30 bg-input/30 shadow-none! *:data-[slot=input-group-addon]:pl-3.5!">
        <CommandPrimitive.Input
          data-slot="command-input"
          className={cn(
            "w-full text-base outline-hidden disabled:cursor-not-allowed disabled:opacity-50 px-2",
            className
          )}
          {...props}
        />
...
function CommandList({
  ...
  return (
    <CommandPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[450px] scroll-py-1 overflow-x-hidden overflow-y-auto outline-none",
        className
      )}
      {...props}
    />
...
function CommandItem({
  ...
  return (
    ...
    >
      {children}
      <CheckIcon className="ml-auto size-4 shrink-0 hidden group-data-[checked=true]/command-item:block" />
    </CommandPrimitive.Item>
  )
}
